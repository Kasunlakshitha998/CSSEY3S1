...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        ...
        axios.get('http://localhost:5000/api/admin/records', {
            headers: {
    ...
    const handleSubmit = (e) => {
        ...
        axios.post('http://localhost:5000/api/admin/records', formData, {
            headers: {
    ...
    const generatePDF = (id) => {
        axios.get(`http://localhost:5000/api/admin/report/${id}`, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}` // Include JWT token in the request
            }
        })
                ...
                <tbody>
                    {records.map(record => (
                        <tr key={record._id}>
                            ...
                            <td>
                                <button className="pdf-button" onClick={() => generatePDF(record._id)}>Download PDF</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
