...
const PatientDashboard = () => {
    ...
    useEffect(() => {
        // Change this line in PatientDashboard
...
;
        ...
        // Fetch prescriptions
        axios.get('http://localhost:5000/api/patient/prescriptions', {
            headers: { 'Authorization': `Bearer ${token}` } // Use Authorization header
        })
        ...
        // Fetch allergies
        axios.get('http://localhost:5000/api/patient/allergies', {
            headers: { 'Authorization': `Bearer ${token}` } // Use Authorization header
        })
    ...
    const handleAllergySubmit = (e) => {
        ...
        if (allergyInput) {
            ...
            axios.post('http://localhost:5000/api/patient/allergies', { name: allergyInput }, {
    ...
    const generatePDF = (id) => {
        axios.get(`http://localhost:5000/api/patient/report/${id}`, {
            headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } // Use Authorization header
        })
        .then(() => {
                ...
                <tbody>
                    {records.map(record => (
                        <tr key={record._id}>
                            ...
                            <td>
                                <button className="view-button" onClick={() => generatePDF(record._id)}>Download PDF</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
                ...
                <div className="allergies-section">
                    ...
                    <ul>
                        {allergies.map(allergy => (
                            <li key={allergy._id} className={allergy.severity === 'high' ? 'high-risk' : 'low-risk'}>
                                <p>{allergy.name}</p>
