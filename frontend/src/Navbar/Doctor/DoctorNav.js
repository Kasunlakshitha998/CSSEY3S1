  ...
  return (
    <>
      <header className="fixed top-0 left-0 w-full p-4 bg-white shadow-md z-10 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4 mr-5 relative">
      ...
      <div className={`fixed inset-y-0 left-0 bg-white shadow-lg z-20 transform transition-transform duration-300 ${sidebarOpen ? 'w-64' : 'w-16'} mt-16`}>
        <div className={`bg-purple-600 p-4 ${sidebarOpen ? 'flex' : 'hidden'} justify-between`}>
          {sidebarOpen && <h2 className="ml-5 text-3xl text-white font-semibold">MENU</h2>}
        </div>
        <ul className="mt-5">
          {navLinks.map((link) => (
